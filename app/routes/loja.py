...
def api_produtos(slug):
    tenant = _get_tenant(slug)
    agora  = datetime.now()
    q = Product.query.filter_by(tenant_id=tenant.id, active=True, online_active=True)\
    ...
    # Na lanchonete, o toggle "Disponível" do Cardápio da Loja controla a vitrine
    if tenant.is_lanchonete:
        q = q.filter(or_(Product.disponivel == None, Product.disponivel == True))
    produtos = q.order_by(Product.sort_order.nulls_last(), Product.name).all()
